...
import MnistPreview from './MnistPreview'
import { loadModel, predict, MNIST_SPEC, type Prediction } from '../lib/mnist'
...
export default function DrawMode() {
  ...
  const frames = useMemo(() => {
    ...
    return runAttack(
      ...
      { kind, epsilon, steps: PGD_STEPS, targetClass: targeted ? targetClass : undefined },
      MNIST_SPEC,
    )
  ...
  return (
    <main className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {status === 'error' && (
        <div className="md:col-span-2 lg:col-span-3 mb-2 rounded-lg border border-red-800 bg-red-950/40 p-4 text-sm text-red-300">
      ...
      <section className="border-line bg-panel rounded-xl border p-5 md:col-span-2 lg:col-span-1">
        ...
        <details className="border-line mt-5 border-t pt-3">
          ...
          <p className="text-muted mt-2 text-xs leading-relaxed">
            <strong className="text-fg">FGSM</strong> (Goodfellow et al., 2014) takes the gradient
            of the model's loss with respect to <em>every pixel</em> and steps once by ε in the
            sign of that gradient: x′ = x + ε·sign(∇ₓL).{' '}
            <strong className="text-fg">PGD</strong> (Madry et al., 2017) repeats that in small
            steps, projecting back into the ε-ball each time — a stronger attack that often flips
            the label at an ε where FGSM can't. <strong className="text-fg">Targeted</strong> mode
            descends the loss toward a class you pick. Every gradient is computed right here in
            your browser.
          </p>
        </details>
      </section>
      ...
      {input && shown && (
        <section className="border-line bg-panel rounded-xl border p-5 md:col-span-2 lg:col-span-3">
          ...
          <p className="text-muted mb-5 text-xs">
            Left to right: what you drew, the adversarial perturbation (amplified so you can see
            it), and their sum — what the model was actually shown.
          </p>
          <div className="flex flex-wrap items-start justify-between gap-8">
            ...
            <div className="min-w-60 flex-1">
              <div className="mb-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
                <span className={`font-mono text-4xl font-bold ${success ? 'text-red-400' : 'text-accent'}`}>
                  {shown.label}
                </span>
